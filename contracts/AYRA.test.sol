...
contract AYRA is ERC20, Ownable {
    ...
    uint256 private _stakingPeriod = block.timestamp + 4 minutes;
    uint256 private _stakingFirstPeriod = block.timestamp + 2 minutes;
    ...
    mapping (address => uint256) _rewardedLastTime;
    ...
    */
    modifier onlyUnblock(address walletAddress) {
        require((walletAddress != walletMarketProtection && walletAddress != walletBlockedFoundingPartners)
                    || block.timestamp > deployedTime + 10 minutes, "This wallet address is blocked for 5 years." );
        _;
    }
    ...
    */
    ...
    function burn() external onlyOperator {
        ...
        require(lastBurnDay + 30 seconds <= block.timestamp, "It's not time to burn. 90 days aren't passed since last burn");
    ...
    /**
     * @dev Stake `amount` tokens from `msg.sender` to `walletStakingAyra`, calculate reward upto now.
     *
    ...
    function stake(uint256 amount) external {
        ...
        // if (!_stakingStarted) {
        //     _stakingPeriod = block.timestamp + 4 minutes;
        //     _stakingFirstPeriod = block.timestamp + 2 minutes;
        // }
    ...
    function getAvailableReward(address account) public view returns (uint256) {
        ...
        if (_rewardedLastTime[account] <= _stakingFirstPeriod) {
            uint256 rewardDays = _stakingFirstPeriod.min(block.timestamp) - _rewardedLastTime[account];
            rewardDays /= 10 seconds;
            reward = rewardDays * _stakedBalances[account] * _stakingFirstPeriodReward / 1000000;
        }

        if (block.timestamp > _stakingFirstPeriod) {
            uint256 rewardDays = _stakingPeriod.min(block.timestamp) - _rewardedLastTime[account].max(_stakingFirstPeriod);
            rewardDays /= 10 seconds;
    ...
    /**
     * @dev Withdraw `amount` tokens from stakingPool(`walletStakingAyra`) to `msg.sender` address, calculate reward upto now.
     *
    ...
    function _updateReward(address account) public {
        uint256 availableReward = getAvailableReward(account);
    ...
    function unlockProtection() public onlyOperator {
        require (block.timestamp > deployedTime + 5 * 2 minutes, "Unlock is not allowed now");
        require (block.timestamp > lastUnlockTime + 2 minutes, "Unlock must be 1 year later from previous unlock");
